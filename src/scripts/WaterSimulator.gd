		...
		for x in range(grid_width):
			...
			for neighbor in neighbors:
				...
				if current_height > neighbor_height:
					...
					var flow = height_diff * diffusion_coefficient * dt * 0.25
...
func _update_visuals() -> void:
	...
	for y in range(grid_height):
		for x in range(grid_width):
			...
			_create_water_visual(x, y, volume)

func _create_water_visual(x: int, y: int, volume: float) -> void:
	...
	material.albedo_color = Color(0.2, 0.5, 1.0, 0.65)
	material.shading_mode = BaseMaterial3D.SHADING_MODE_UNSHADED
	material.transparency = BaseMaterial3D.TRANSPARENCY_ALPHA
	mesh.material = material
	
	water.mesh = mesh
	water.position = Vector3(world_pos.x, visual_height + mesh_height / 2 + 0.05, world_pos.z)
