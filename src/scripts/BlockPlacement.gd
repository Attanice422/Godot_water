...
func _place_wall(x: int, y: int) -> void:
	...
	mesh.size = Vector3(grid_manager.cell_size * 0.8, 0.8, grid_manager.cell_size * 0.8)
	
	var material = StandardMaterial3D.new()
	material.albedo_color = Color(0.3, 0.3, 0.35, 1.0)
	material.shading_mode = BaseMaterial3D.SHADING_MODE_UNSHADED
	mesh.material = material
	
	wall.mesh = mesh
	wall.position = Vector3(world_pos.x, visual_height + 0.45, world_pos.z)
